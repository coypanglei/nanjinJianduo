package com.shaoyue.weizhegou.widget.lineTU;
...
import static com.sunfusheng.marqueeview.Utils.dip2px;
import static com.sunfusheng.marqueeview.Utils.sp2px;
...
public class LineView extends View {
    ...
    private final int BOTTOM_TEXT_COLOR = Color.parseColor("#1c1a1d");
    ...
    private Paint popupLeftText = new Paint();
    private boolean showFloatNumInPopup;
    private Dot pointToSelect;
    private Dot selectedDot;
    private Double xjlNum = 1.00;

    public Double getXjlNum() {
        return xjlNum;
    }

    public void setXjlNum(Double xjlNum) {
        this.xjlNum = xjlNum;
    }

    private int popupBottomPadding = dip2px(getContext(), 2);
    /*
          |  | ←topLineLength
        --+--+--+--+--+--+--
        --+--+--+--+--+--+--
    ...
    private int sideLineLength = dip2px(getContext(), 60) / 3 * 2;

    private int backgroundGridWidth = dip2px(getContext(), 65);
    private int showPopupType = SHOW_POPUPS_NONE;
    private Boolean drawDotLine = false;
    private int[] colorArray = {
            Color.parseColor("#e74c3c"), Color.parseColor("#23bf86"), Color.parseColor("#cc6bcc")
    };
    ...
    public LineView(Context context, AttributeSet attrs) {
        ...
        popupLeftText.setAntiAlias(true);
        popupLeftText.setTextSize(sp2px(getContext(), 12));
        popupLeftText.setTextAlign(Paint.Align.CENTER);
        popupLeftText.setStyle(Paint.Style.FILL);
        popupLeftText.setColor(BOTTOM_TEXT_COLOR);
     ...
     */
    public void setFloatDataList(ArrayList<ArrayList<Double>> dataLists) {
        ...
        if (dataLists != null && !dataLists.isEmpty()) {
            ...
            for (int k = 0; k < dataLists.size(); k++) {
                ...
                for (int i = 0; i < dataLists.get(k).size(); i++) {
                    ...
                    double y = getYAxesOf(dataLists.get(k).get(i), verticalGridNum);
                    if (i > drawDotSize - 1) {
    ...
    private double getYAxesOf(double value, int verticalGridNum) {
     ...
     *               0 1 2 3 4 5
     */
    private void drawPopup(Canvas canvas, double num, Point point, int PopupColor) {
        String numStr = showFloatNumInPopup ? String.valueOf(num) + "万" : String.valueOf(Math.round(num)) + "万";
        ...
        if (drawDotLists != null && !drawDotLists.isEmpty()) {
            for (int k = 0; k < drawDotLists.size(); k++) {
                ...
                for (Dot dot : drawDotLists.get(k)) {
                    canvas.drawCircle(dot.x, (float) dot.y, DOT_OUTER_CIR_RADIUS, bigCirPaint);
                    canvas.drawCircle(dot.x, (float) dot.y, DOT_INNER_CIR_RADIUS, smallCirPaint);
                }
            }
        }
    }
    ...
    private void drawLines(Canvas canvas) {
        ...
        for (int k = 0; k < drawDotLists.size(); k++) {
            ...
            for (int i = 0; i < drawDotLists.get(k).size() - 1; i++) {
                canvas.drawLine((float) drawDotLists.get(k).get(i).x, (float) drawDotLists.get(k).get(i).y,
                        drawDotLists.get(k).get(i + 1).x, (float) drawDotLists.get(k).get(i + 1).y,
                        linePaint);
            }
        }
    }
    ...
    private void drawBackgroundLines(Canvas canvas) {
        ...
        for (int i = 0; i < yCoordinateList.size(); i++) {
            if ((yCoordinateList.size() - 1 - i) % dataOfAGird == 0) {
                dottedPath.moveTo(115, yCoordinateList.get(i));
                dottedPath.lineTo(getWidth(), yCoordinateList.get(i));
                canvas.drawPath(dottedPath, paint);
                mlist.add(yCoordinateList.get(i));
            }
        }
        Collections.reverse(mlist);
//        LogUtils.e(dataOfAGird);
        for (int i = mlist.size() - 1; i >= 0; i--) {
            String str = i * dataOfAGird + "";
            canvas.drawText(str
                    , 50,
                    mlist.get(i), bottomTextPaint);
        }
    ...
    class Dot {
        ...
        Dot(int x, double y, int targetX, double
                targetY, double data, int linenumber) {
            this.x = x;
            this.y = y;
        ...
        Dot setTargetData(int targetX, double targetY, double data, int linenumber) {
        ...
        private double updateSelf(double origin, double target, int velocity) {
            if (origin < target) {
                origin += velocity;
