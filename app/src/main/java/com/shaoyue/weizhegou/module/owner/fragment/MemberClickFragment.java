 ...
 * 邮箱：434604925@qq.com
 */
public class MemberClickFragment extends BaseTitleFragment {
    ...
    String[] mTextviewArray = {"所有会员", "一级", "二级", "三级"};
    ...
    public static MemberClickFragment newInstance(profileBean mProfileContent) {

        Bundle args = new Bundle();
        args.putString("click", mProfileContent.getContent());
        args.putStringArray("titles", mProfileContent.getmString());
        MemberClickFragment fragment = new MemberClickFragment();
...
//        indicatorViewPager.setCurrentItem(Integer.parseInt(click), false);
    ...
    public void refreshNum(RefreshNumEvent event) {
        if (event.getType() == 0) {
            mTextviewArray[0] = "所有会员(" + event.getNum() + ")";
            ...
            mTextviewArray[3] = "三级(" + event.getNum() + ")";
            myAdapter.getIndicatorAdapter().notifyDataSetChanged();
        }
    }
