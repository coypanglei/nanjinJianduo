...
public class DgdkjcFragment extends BaseAppFragment implements BGARefreshLayout.BGARefreshLayoutDelegate {
    ...
    private DgdkjcAdapter mAdapter;


    public static DgdkjcFragment newInstance(String title) {

        Bundle args = new Bundle();
        args.putString("title", title);
        DgdkjcFragment fragment = new DgdkjcFragment();
    ...
    protected int getLayoutId() {
        return R.layout.fragment_dh_dg;
    }
    ...
    protected void initView(View rootView) {
        ...
        mAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                List<DgInfoListBean.RecordsBean> mlist = adapter.getData();
                for (DgInfoListBean.RecordsBean data : mlist) {
                    data.setClick(false);
                }
    ...
    public void onMessageEvent(OkOrCancelEvent event) {
        if (event.getmType().equals("是否取消申请?")) {
            DgInfoListBean.RecordsBean item = null;
            for (DgInfoListBean.RecordsBean data : mAdapter.getData()) {
                if (data.isClick()) {
                    item = data;
                }
            }
    ...
    private DgInfoListBean.RecordsBean getSelect() {
        for (DgInfoListBean.RecordsBean data : mAdapter.getData()) {
            if (data.isClick()) {
                return data;
            }
...
//                        SPUtils.getInstance().put(UserMgr.SP_APPLY_ID, getSelect().getQyzjhm());
    ...
    private void initData() {
        ...
        DhApi.getDgJbList(page, "12", mNameOrId, new BaseCallback<BaseResponse<DgInfoListBean>>() {
            @Override
            public void onSucc(BaseResponse<DgInfoListBean> result) {
    ...
    public boolean onBGARefreshLayoutBeginLoadingMore(BGARefreshLayout refreshLayout) {
        ...
        DhApi.getDgJbList(page + 1, "12", mNameOrId, new BaseCallback<BaseResponse<DgInfoListBean>>() {
            @Override
            public void onSucc(BaseResponse<DgInfoListBean> result) {
                pages = result.data.getPages();
