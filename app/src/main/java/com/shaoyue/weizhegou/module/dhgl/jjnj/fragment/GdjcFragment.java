...
import com.shaoyue.weizhegou.module.credit.adapter.diaocha.GdjcAdapter;
...
public class GdjcFragment extends BaseAppFragment implements BGARefreshLayout.BGARefreshLayoutDelegate {
    ...
    private GdjcAdapter mAdapter;


    public static GdjcFragment newInstance(String title) {

        Bundle args = new Bundle();
        args.putString("title", title);
        GdjcFragment fragment = new GdjcFragment();
    ...
    protected int getLayoutId() {
        return R.layout.fragment_dh_gd;
    }
    ...
    protected void initView(View rootView) {
        ...
        mAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                List<GdInfoListBean.RecordsBean> mlist = adapter.getData();
                for (GdInfoListBean.RecordsBean data : mlist) {
                    data.setClick(false);
                }
    ...
    public void onMessageEvent(OkOrCancelEvent event) {
        if (event.getmType().equals("是否取消申请?")) {
            GdInfoListBean.RecordsBean item = null;
            for (GdInfoListBean.RecordsBean data : mAdapter.getData()) {
                if (data.isClick()) {
                    item = data;
                }
            }
    ...
    private GdInfoListBean.RecordsBean getSelect() {
        for (GdInfoListBean.RecordsBean data : mAdapter.getData()) {
            if (data.isClick()) {
                return data;
            }
            ...
            case R.id.sb_tianbao:
                if (ObjectUtils.isNotEmpty(getSelect())) {
                    if (ObjectUtils.isEmpty(getSelect().getId())) {   //请求id 身份证
                        ...
                        UIHelper.showDcCommonActivity("个贷", getActivity(), "个贷检查");
                    }
                } else {
    ...
    private void initData() {
        ...
        DhApi.getGdJbList(page, "12", mNameOrId, new BaseCallback<BaseResponse<GdInfoListBean>>() {
            @Override
            public void onSucc(BaseResponse<GdInfoListBean> result) {
    ...
    public boolean onBGARefreshLayoutBeginLoadingMore(BGARefreshLayout refreshLayout) {
        ...
        DhApi.getGdJbList(page + 1, "12", mNameOrId, new BaseCallback<BaseResponse<GdInfoListBean>>() {
            @Override
            public void onSucc(BaseResponse<GdInfoListBean> result) {
                pages = result.data.getPages();
